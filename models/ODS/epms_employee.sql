...
{% if is_incremental() %}
{% do run_query(incremental_delete_prehook('snap_stg_epms_employee', unique_keys)) %}
{% endif %}
...
{{ dbt_utils.star(
from=ref('snap_stg_epms_employee'),
...
FROM {{ ref('snap_stg_epms_employee') }} snapshot_data
